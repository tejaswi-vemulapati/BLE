...
class BLEManager: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate{
    ...
    // Peripheral Data
    var peripheralManagerBLE: CBPeripheralManager!
    ...
    override init() {
        ...
        peripheralManagerBLE = CBPeripheralManager()
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?){
        /*
        print("Peripheral Delegate Initiated")
        if let service = peripheral.services?.first(where: { $0.uuid == serviceUUID}){
            peripheral.discoverCharacteristics([characteristicsUUID], for: service) // For MouseCharacteristic UUID
 ...
 */
    ...
    private func sendData() {
        ...
        guard let transferCharacteristic = transferCharacteristicBLE else {
            return
        }
        ...
        if sendingEOM {
            // send it
            let didSend = peripheralManagerBLE.updateValue("EOM".data(using: .utf8)!, for: transferCharacteristic, onSubscribedCentrals: nil)
            // Did it send?
            if didSend {
        ...
        while didSend {
            ...
            // Send it
            didSend = peripheralManagerBLE.updateValue(chunk, for: transferCharacteristic, onSubscribedCentrals: nil)
            ...
            if sendDataIndex >= dataToSend.count {
                ...
                //Send it
                let eomSent = peripheralManagerBLE.updateValue("EOM".data(using: .utf8)!,
    ...
    private func setupPeripheral() {
        ...
        let transferService = CBMutableService(type: characteristicsUUID, primary: true)
        
        // Add the characteristic to the service.
        transferService.characteristics = [transferCharacteristic]
        
        // And add it to the peripheral manager.
        peripheralManagerBLE.add(transferService)
        
        // Save the characteristic for later.
        transferCharacteristicBLE = transferCharacteristic
        
    }
    internal func peripheralManagerDidUpdateState(_ peripheral: CBPeripheralManager, strValue: String){
        
        isAdvertisingBLE = peripheral.state == .poweredOn
    ...
    func peripheralManager(_ peripheral: CBPeripheralManager, central: CBCentral, didSubscribeTo characteristic: CBCharacteristic) {
        ...
        // Get the data
        dataToSend = "Hi! This is a Test!".data(using: .utf8)!
